        ...
        <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((star) => (
                <Star
                    key={star}
                    size={24}
                    className={`cursor-pointer transition-all duration-150 ${star <= (hoverRating || rating)
...
const VoicesAndSupport = () => {
    ...
    // ----- Submit handler with email fallback -----
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const safeReview = {
                ...newReview,
                email: newReview.email || finalEmail,
                likes: newReview.likes ?? 0,
                timestamp: newReview.timestamp || new Date().toISOString(),
            };
            setReviews((prev) => [safeReview, ...(Array.isArray(prev) ? prev : [])]);
    ...
    // ----- Helper: mask email (safe) -----
    const maskEmail = (email) => {
        if (!email) return 'anonymous@user.com';
